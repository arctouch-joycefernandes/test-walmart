  ...
  this.execute(this.executeSizzlejs, [this.selector, this.injectedJsCommand()], function (result) {
    ...
    if (result.seens >= JS_SEEN_MAX || self.seenCount >= SEEN_MAX || elapsed > MAX_TIMEOUT) {
      ...
      if (self.seenCount >= SEEN_MAX || result.seens >= JS_SEEN_MAX) {
        ...
        self.fail({ code: _settings2.default.FAILURE_REASONS.BUILTIN_SELECTOR_NOT_VISIBLE });
      } else {
...
Base.prototype.execute = function (fn, args, callback) {
  ...
  this.nightwatchExecute(fn, innerArgs, function (result) {
    ...
    if (result && result.status === 0 && result.value !== null) {
      ...
      self.fail({ code: _settings2.default.FAILURE_REASONS.BUILTIN_SELENIUM_ERROR });
    }
  });
};

Base.prototype.pass = function (_ref) {
  var pactual = _ref.pactual,
  ...
  this.client.assertion(true, pactual, expected, _util2.default.format(fmtmessage, this.time.totalTime), true);
...
Base.prototype.fail = function (_ref2) {
  ...
  this.time.totalTime = new Date().getTime() - this.startTime;
  var fmtmessage = "" + (this.isSync ? "[sync mode] " : "") + this.message + " [[" + pcode + "]]";

  this.client.assertion(false, pactual, expected, _util2.default.format(fmtmessage, this.time.totalTime), true);
  this.emit("complete");
};
