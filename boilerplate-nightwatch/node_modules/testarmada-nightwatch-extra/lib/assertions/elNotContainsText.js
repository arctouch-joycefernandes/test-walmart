...
var ElNotContainsText = function ElNotContainsText() {
...
_util2.default.inherits(ElNotContainsText, _baseAssertion2.default);

ElNotContainsText.prototype.assert = function (actual, expected) {
  if (expected !== undefined && actual.indexOf(expected) < 0 && !new RegExp(expected).exec(actual)) {
    this.pass({
      pactual: actual,
      expected: expected,
    ...
    this.fail({
      code: _settings2.default.FAILURE_REASONS.BUILTIN_ACTUAL_NOT_MEET_EXPECTED,
      pactual: actual,
      expected: expected,
      message: this.message
    });
...
ElNotContainsText.prototype.command = function (selector, expected) {
  ...
  this.message = _util2.default.format("Testing if selector <%s> does not contains text <%s> after %d milliseconds ", this.selector, this.expected);
...
//# sourceMappingURL=elNotContainsText.js.map