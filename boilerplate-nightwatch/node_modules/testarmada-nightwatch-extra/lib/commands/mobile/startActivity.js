...
var StartActivity = function StartActivity() {
...
_util2.default.inherits(StartActivity, _baseMobileCommand2.default);
...
StartActivity.prototype.checkConditions = function () {
  ...
  self.protocol(options, function (result) {
    ...
    if (result.status === 0) {
      // sucessful
      self.seenCount += 1;
    } else if (result.status === -1 && result.errorStatus === 13) {
      // method not implement, fail immediately
      self.fail({
        code: _settings2.default.FAILURE_REASONS.BUILTIN_COMMAND_NOT_SUPPORTED,
    ...
    if (self.seenCount >= 1 || elapsed > MAX_TIMEOUT) {
      if (self.seenCount >= 1) {
        ...
        self.fail({
          code: _settings2.default.FAILURE_REASONS.BUILTIN_COMMAND_TIMEOUT,
          message: self.failureMessage
        });
      }
    } else {
...
StartActivity.prototype.command = function (app, cb) {
  ...
  this.successMessage = "Activity " + this.appPackage + "." + this.appActivity + " was started after %d milliseconds";
  this.failureMessage = "Activity " + this.appPackage + "." + this.appActivity + " wasn't started after %d milliseconds";
