...
ScrollDownToElement.prototype.command = function (xStart, yStart, elementLocateStrategy, elementSelector, cb) {
  ...
  var scrollToFindElement = function scrollToFindElement(attempts) {
    if (attempts < MAX_ATTEMPTS) {
      self.client.api.getMobileElConditional(elementLocateStrategy, elementSelector, CONDITIONAL_CHECK, function (result) {
        if (!result) {
          var yEnd = yStart > distance ? yStart - distance : 1;
          var xEnd = xStart;
          if (_lodash2.default.toLower(self.client.api.capabilities.platformName) === "android") {
            // do nothing here
          } else if (_lodash2.default.toLower(self.client.api.capabilities.platformName) === "ios") {
